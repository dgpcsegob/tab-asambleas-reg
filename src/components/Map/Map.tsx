...
const Map: React.FC<MapProps> = ({ layersVisibility }) => {
  ...
  const attachAllTooltipEvents = useCallback((map: MaplibreMap) => {
    ...
    const checkMeasurement = () => isMeasuringRef.current;
    ...
    zonas.forEach(zona => {
      map.on('mouseenter', `regiones_${zona}`, (e) => {
        if (checkMeasurement() || !e.features || e.features.length === 0) return;
        map.getCanvas().style.cursor = 'pointer';
        const props = e.features[0].properties;
        if (props) {
            const tooltipHtml = `<div style="text-align: left;"><strong>Región de Paz</strong><br/><strong>Entidad:</strong> ${props._NOM_ENT || 'N/A'}<br/><strong>Municipio:</strong> ${props.NOMGEO || 'N/A'}<br/><strong>Región:</strong> ${props._REGION || 'N/A'}<br/><strong>Nombre:</strong> ${props._NOM_REGION || 'N/A'}</div>`;
            popup.setLngLat(e.lngLat).setHTML(tooltipHtml).addTo(map);
        }
      });
    ...
    tecnologias.forEach(id => {
      map.on('mouseenter', id, (e) => {
        ...
        const props = e.features[0].properties;
        if (props) {
            popup.setLngLat(e.lngLat).setHTML(`<strong>Nombre:</strong> ${props['INMUEBLE NOMBRE']}<br/><strong>Tecnología:</strong> ${props['TECNOLOGIA']}`).addTo(map);
        }
      });
  ...
  const addVectorLayers = (map: maplibregl.Map) => {
    ...
    zonas.forEach(zona => {
      ...
      if (!map.getLayer(`regiones_${zona}`)) {
        map.addLayer({
          id: `regiones_${zona}`, type: 'fill', source: `regiones_${zona}`, 'source-layer': `regiones_${zona}_tile`,
          paint: { 'fill-color': matchExpression, 'fill-opacity': 0.5, 'fill-outline-color': '#333333' }
        });
      }
    });
    ...
    if (!map.getLayer('LocalidadesSedeINPI')) {
      map.addLayer({
        ...
        paint: { 'circle-radius': 1.4, 'circle-color': puebloExpression, 'circle-stroke-color': '#ffffff', 'circle-stroke-width': 0 }
      });
    }
    ...
    if (!map.getLayer('PresidenciasMunicipales')) {
      map.addLayer({
        ...
        paint: { 'circle-radius': 1.3, 'circle-color': '#000000' }
      });
    }
    ...
    tecnologias.forEach(({ id, color, filtro }) => {
      if (!map.getLayer(id)) {
        map.addLayer({
          ...
          paint: { 'circle-radius': 1.2, 'circle-color': color, 'circle-stroke-color': '#ffffff', 'circle-stroke-width': 0 }
        });
      }
    });
  };
  ...
  useEffect(() => {
    ...
    map.on('load', () => {
      ...
      const asambleasRegionalesLayers = ['puntos_zona1', 'puntos_zona2'];
